...
1. **Decide whether this skill applies.** Read `.uxproof/contract.json`, `tokens.json`, and the code when present. Distinguish this skill's declared contract (`source: "design"`, its manual design block and `${design}/theme.css`) from a design system implemented in product code. With no implemented system, `--refresh` may revise that declared contract; without the flag, keep it and offer `--refresh` instead of creating a second one. Its theme file and its planned component list alone do not trigger extraction. Tokens extracted from product styles/theme modules, implemented shared components, or a UI library used by the screens mean a real design system exists → route to `om-ux-setup` (`--refresh` for re-extraction), even if declared tokens are also present. A proposed palette or a repository with neither kind of contract follows the creation path: the palette is one input, never the answer.
