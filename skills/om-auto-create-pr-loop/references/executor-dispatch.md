...
Applies only to **Spec-implementation runs** with **many Steps that must ship in one PR** — Simple runs have at most one code commit and never dispatch.
...
Use it for long runs with many Steps, or whenever the main session would otherwise carry heavy per-Step context. Do NOT use it for short runs (1–2 Steps — drive them directly with the default per-Step loop) or docs-only/trivial runs.
...
Every 5 successful executors (or when a Phase with ≥3 Steps closes), the main session MUST run a full **checkpoint pass** per step 8 (`references/checkpoint-pass.md`) before dispatching the next Step.
