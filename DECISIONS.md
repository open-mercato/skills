...
## 2026-08-13 — Reporting is template-based with mandatory fields and a maximum

**Supersedes the "Reporting is template-based and deliberately un-laconic" bullet of 2026-07-23** (kept above, annotated, so the reasoning that produced it stays readable). **Approved by Wojciech Szyjka on 2026-08-13**, on the design PR that requested it — the amendment touches a decision the whole collection cites, so it was taken by a person rather than defaulted by an autonomous run.

The 2026-07-23 decision diagnosed the problem correctly and reached for the wrong lever. What made Codex output poor was the absence of an enforced *structure*, not the absence of *volume*: report shapes were inline prose carrying the word "concise", so a terse runtime had nothing to fill in. The fix chosen then was templates **plus** a binding "never compress" rule, and the second half turned out to be load-bearing in the wrong direction — it licensed padding, and it made the code-review body the single artifact serving both the reviewing agent and every human watching the PR. The reported cost is in the record: engineers stopped reading review comments (OM Core channel, 2026-08-11/12).

The corrected rule: **fill every mandatory field, then stop.** What protects against runtime divergence is the template's mandatory fields, and a *maximum* protects them further — a runtime that pads to look thorough fails the budget as visibly as a runtime that omits a field fails the template. A volume floor was never the mechanism; it was a proxy for one. Concretely:

- Templates stay binding, and so does deliverable-not-log: full sentences, the *why* behind every verdict, label, and finding, and the shared emoji glossary. Nothing here relaxes those.
- "Never compress the report to save tokens" is replaced by a per-template character budget with overflow folded into `<details>`, never deleted. A report that cannot fit is a report with too many findings, and it says so.
- Sections render **conditionally**: a checklist with nothing to declare, or a coverage narrative with no gap, is absence of signal, not brevity.
- Agent-facing and human-facing projections of the same report may differ. The full report remains the chain-facing artifact; the posted comment is a budgeted projection of it. This split is what makes a maximum safe — nothing is lost, only re-addressed.
- Machine-parsed chain contracts stay plain and stay out of the budget, exactly as before.

Where this lands in the skills is scoped by the spec that carries it (`.ai/specs/2026-08-12-review-noise-reduction.md`, Phase 2): `om-code-review`'s `output-format.md` and `SKILL.md`, `om-auto-review-pr`'s `verdict-and-labels.md` and `SKILL.md`, the "Reporting style" bullet in the four review-posting skills' `rules.md`, `AGENTS.md` (cross-skill contract §3 and skill-authoring standard §2), and `CODE_REVIEW.md` priority 8 with its severity guidance — which today makes a terser-than-template report a review finding and would otherwise flag the very projection this decision authorizes. The other 30 `rules.md` copies are deliberately left for a later sweep: an unsynced copy keeps today's behaviour, so under-syncing is safe.

**Reverting** is a documentation change with no migration: restore the "never compress" wording in those files and the earlier bullet governs again.
