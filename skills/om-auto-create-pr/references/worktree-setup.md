...
## Pin this skill's directory before you `cd`

The worktree created below is a checkout of the **target project**; it may not contain the installed `om-*` skills. Before changing directories, retain the absolute directory of this skill's loaded `SKILL.md`. Resolve this skill's later files from that directory, for example `<skill-dir>/references/worktree-setup.md`.

Keep invoking sibling `om-*` skills by name so the runner resolves them. Never guess a runner-specific skills root, read another skill's `references/` directly, record an install path in `.ai/agentic.config.json`, or copy/symlink skills into the worktree.
