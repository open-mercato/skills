...
After confirmation, the skill offers the next useful action. A ready brief can go to a backlog dry run. An optional [om-synthetic-users](om-synthetic-users.md) panel requires a concrete Key flow; its output enters the brief through a confirmed refresh. Without a flow, the panel is skipped. A running-product check passes `--app`; a brief walkthrough is explicitly narrative. The selected flow and research directory are passed with the handoff. Companion skills run only when authorized. `Next:` names an authorized action still waiting to run; declined, unaccepted and completed actions yield `none`. On refresh, new tracker decisions enter before drafting and review. Tracker access is read-only, and this skill does not commit or publish files.
