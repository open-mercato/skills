...
description: Resume an in-progress PR started by `om-auto-create-pr`. Claims the PR, checks the branch into an isolated worktree, reads the linked execution plan's Progress checklist, continues from the first unchecked step. Reframes a doc-originated spec PR into a feature PR when the resume lands implementation code. Usage - /om-auto-continue-pr <PR-number>
---
...
5. **Resume execution.** From the resume point forward, apply the **same phase-by-phase loop** documented in the `om-auto-create-pr` skill:
...
7. **Self-review.** Run the `om-code-review` skill against the branch diff and the breaking-change checklist: no silently broken public contract (checked against `BACKWARD_COMPATIBILITY.md` when present), no removed API response fields, no weakened security-sensitive surface, scope still matching the plan with no churn introduced by the resume. Fix findings and loop back to step 5. Full checklist: `references/review-report.md`.

8. **Run `om-auto-review-pr` and apply fixes.** Before the final summary comment, the last pushes, or the `complete` flip, subject the resumed PR to an automated second pass with `om-auto-review-pr` in autofix mode (its claim check recognizes the current user already owns the step-1 `in-progress` lock and proceeds as re-entry). Follow its workflow verbatim: fixes as new commits in the same worktree (never rewriting history), re-run targeted validation (full step-6 gate when a fix reaches beyond a single module/test file), update the plan's Progress, and loop until a clean verdict or only documented non-actionable findings remain. If it cannot run (checks not green, missing context), stop, leave `Status: in-progress`, and document the blocker. Full procedure and verdict handling: `references/review-report.md`.

9. **Post the comprehensive summary comment.** Every resume MUST end with a single, comprehensive summary comment on the PR that captures what this resume changed on top of the previous state, posted via **comment-pr** with a body file so formatting is preserved. Use the full structure — Summary of changes in this resume, External references honored, Verification phases completed, How to verify, What can go wrong — and its rules from `references/summary-comment-template.md`. Never post it before step 8 finishes, never claim a completion you did not reach, and never paste secrets into it.

10. **Update the PR, normalize labels, release the lock, clean up.** Follow `references/pr-finalize.md`: this step **updates the existing PR** — it never opens a new one; prefer the `om-open-pr` skill for the push + label-normalization mechanics when installed, inline tracker operations when not. First, **reframe a doc-originated PR (spec → feature)** when this resume (or an earlier one) landed implementation code on a spec/design-only PR: rewrite title/body via **update-pr**, swap the `documentation`/`skip-qa`/`risk-low` labels, keep the original description for the record (trigger + procedure: `references/pr-finalize.md`). Then update the PR body (flip `Status: in-progress` to `Status: complete` when all Progress steps are `- [x]` — and **flip the PR itself from draft to ready via mark-pr-ready** at that same point, since `om-auto-create-pr` leaves the PR a draft while unfinished; a resume that stays `in-progress` leaves it a draft; extend `What Changed` / `Tests` with this resume's work) and apply the resume label semantics through the guards: keep non-terminal pipeline states, add `needs-qa` for newly user-facing work (dropping stale `qa-approved`), preserve or justifiably raise priority and risk, comment after every label change. Then release the `in-progress` lock — **always**, even on failure (trap/finally; **unlabel-pr** + completion comment per `references/claim-pr.md`) — and remove the worktree you created (`references/worktree-setup.md`).

11. **Report back.** Summarize to the user:
    ...
    If the resume still did not reach `complete`, leave `Status: in-progress` in the PR body and tell the user how to re-enter. End the report with the chaining reference lines — `PR: #<number> (link: <url>)`, plus `Issue: #<number> (link: <url>)` when the run has a subject issue — so the next skill in a chain can consume them.
...
- After the resume's targeted/full validation passes, run the `om-auto-review-pr` skill against the PR in autofix mode and keep applying fixes (as new commits, never as history rewrites) until it returns a clean verdict or only non-actionable findings remain — before posting the summary comment, pushing the final changes, and reporting back.
- Every resume MUST end with the single comprehensive summary comment of step 9, with stable section headings across runs.
- Reframe a doc-originated PR (title/body/labels) via **update-pr** when a resume lands implementation code on a spec/design-only PR — never leave a feature under a `docs(specs):` title. Idempotent; keeps the original description for the record (`references/pr-finalize.md`).
