...
Options:
  ...
  --mock-spec <path>    generate a mock repo from a JSON scenario spec
                        ({ files, branches, goal?, outcomeProperties? }).
...
// Shared fixture: a coupon-stacking function with deliberately planted bugs.
// The `review` scenario asks a skill to find them; the `implement` scenario
// asks a skill to fix/harden them.
const COUPONS_BUGGY =
...
// Built-in scenarios, layered over the base project. A scenario may declare a
// `goal` + `outcomeProperties` (for code-modifying/implementation skills) and/or
// `branches` whose `plantedFindings` seed review-type skills.
const MOCK_SCENARIOS = {
  ...
  implement: () => ({
    goal: 'Harden stackCoupons in src/coupons.js: reject a non-array `coupons` '
      + 'argument, skip entries with a non-numeric `amount`, never throw when the '
      + "caller omits `order.applied` (initialize or guard it), fix the off-by-one "
      + 'so it never reads past the end, and add node:test coverage for the '
      + 'function. Keep `npm test` green.',
    outcomeProperties: [
      'Validates input: rejects or guards a non-array `coupons` argument instead of crashing.',
      'Guards `order.applied`: does not throw when the caller omits it (initializes or checks first).',
      'Iterates correctly: no off-by-one; never reads past the end of `coupons`.',
      'Adds tests: ships node:test coverage for stackCoupons, and `npm test` stays green.',
      'Does not silently swallow errors that would hide a real bug.',
    ],
    // Ship the buggy function on the base so it exists to be hardened.
    files: { 'src/coupons.js': COUPONS_BUGGY },
    // A task branch for the skill to work on (no diff vs main until the skill edits).
    branches: [{ name: 'task/harden-coupons', commitMessage: 'chore: scaffold coupon hardening task', files: {} }],
  }),
}
