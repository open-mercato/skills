...
- **The product skills do not require the setup.** `om-discover`, `om-synthetic-users`, `om-backlog --dry-run`, and `om-ux-style` run as before in a repository without the block; `om-discover`'s report and `om-backlog`'s tree header mention `om-discovery-setup` once as the way to get the gates. `om-discovery-setup` is the only product-layer skill that runs the delivery setup when it is missing.
...
- **Migration:** a repository with an `SDLC.md` that already carries the three sections (generated from this branch before this change) keeps them as is — the skills read the section, not the markers. To bring it under `om-discovery-setup`'s management, run `/om-discovery-setup`: it detects the unmarked sections at their anchors, offers to wrap them, and writes the `discovery` block. `om-apply-upgrade-notes` reports `om-discovery-setup --refresh` for the marked blocks instead of splicing them. The roster in `om-setup-agent-pipeline`'s coverage check gains `om-discovery-setup`.
...
- **Migration:** nothing to do in a repository without `product-brief.md`. A generated `SDLC.md` gains the section *Product decisions as a protected contract* when the product layer is on; `/om-discovery-setup` adds it to an existing one.
