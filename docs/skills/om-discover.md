...
The final `Next:` line is executable routing: it names only an explicitly chosen invocation that has not run yet, with its exact arguments. Executed and declined steps are described in prose and yield `Next: none`; completing a backlog dry run does not authorize filing issues.

Writes the brief that [om-brainstorm](om-brainstorm.md) reads in its Frame step, [om-spec-writing](om-spec-writing.md) reads for its Problem Statement and Open Questions, and [om-prepare-issue](om-prepare-issue.md) reads to fill the ticket-level tier of the Definition of Ready in `SDLC.md`. Its Non-goals, Business rules, and Decisions become a protected contract that [om-code-review](om-code-review.md) and [om-ux-review-pr](om-ux-review-pr.md) enforce, per `SDLC.md`. Loads the design contract from [om-ux-setup](om-ux-setup.md) when present. Tracker access is read-only; it never files, comments, or claims anything — its `Next:` line routes to the skill that does. After the brief is written it offers the next step itself, one yes/no at a time: an optional synthetic panel through [om-synthetic-users](om-synthetic-users.md) on the first key flow, an optional neutral clickable prototype through [om-mockup-prototype](om-mockup-prototype.md), an optional walkthrough of those screens, then its own `--refresh`, and, when the ticket-level Definition of Ready is met, the backlog dry run through [om-backlog](om-backlog.md); otherwise one more decision round. Housekeeping (where the brief lands, who owns it) is settled in one line, never as a round question. Questions are asked in the user's language and in plain words, with the reason each is asked. Available facts are checked first; experiences are explored without suggested answers; decisions get options with consequences and a conditional recommendation only when the context supports one. Examples show a neutral answer shape. The skeptic's findings come back as questions without recommendations or severity labels. A batch confirmation covers explicit choices with their consequences shown; it never confirms missing facts or unanswered prerequisites.
