...
Writes the brief that [om-brainstorm](om-brainstorm.md) reads in its Frame step, [om-spec-writing](om-spec-writing.md) reads for its Problem Statement and Open Questions, and [om-prepare-issue](om-prepare-issue.md) reads to fill the ticket-level tier of the Definition of Ready in `SDLC.md`. Loads the design contract from [om-ux-setup](om-ux-setup.md) when present. Tracker access is read-only; it never files, comments, or claims anything — its `Next:` line routes to the skill that does. After the brief is written it offers the next step itself, one yes/no at a time: an optional synthetic panel through [om-synthetic-users](om-synthetic-users.md) on the first key flow (then its own `--refresh`), and, when the ticket-level Definition of Ready is met, the backlog dry run through [om-backlog](om-backlog.md); otherwise one more decision round. Housekeeping (where the brief lands, who owns it) is settled in one line, never as a round question.
