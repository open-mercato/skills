...
| `om-prepare-test-env` | Prepares a reusable test environment for the app: discovers the repo's own ephemeral/test tooling or generates disposable Docker-based services, reuses running environments (PID-checked, health-probed) and caches builds behind freshness fingerprints, and writes a shared descriptor other skills attach to. Records every bootstrap lesson so the next boot is faster. |
| `om-integration-tests` | Creates and runs integration/E2E tests by exploring the running app first — real locators, runtime fixtures, no hardcoded IDs — and reports failures with artifact-based per-test diagnosis. Discovers how to run your app from the repo itself and reuses the environment `om-prepare-test-env` left behind. |
