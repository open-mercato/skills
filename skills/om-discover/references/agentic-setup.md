...
- **Product layer optional.** When the config has no `discovery.enabled: true`, this skill runs exactly as it would with it; the report's *Next step* paragraph adds one sentence naming `om-discovery-setup` as the way to get the readiness gate at Intake, the product roles in `SDLC.md`, and the protected decisions in review. Never run it from here.
