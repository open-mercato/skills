...
🔁 **Next step.** {what the hand-off offered, what the user declined or chose, and what ran with its artifact paths; describe completed work in prose, without copying child routing lines. Name an outstanding hand-off only if the user explicitly chose it for later execution. When the config has no `discovery.enabled`, one sentence: the product layer is not set up, and `/om-setup-discovery-pipeline` adds the readiness gate, the product roles, and the protected decisions to `SDLC.md`.}
