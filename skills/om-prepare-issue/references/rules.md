...
- This skill defines no chaining markers of its own; it *consumes* the `SPEC_PATH` + `PR_NUMBER` markers emitted by `om-auto-write-spec` on the spec-authoring path and reports the spec path and PR in its final report.
