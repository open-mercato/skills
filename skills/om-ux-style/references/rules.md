...
- **Interactive run — a user is in the loop.** This skill acts once, asks what only the team can answer, reports, and hands control back. It is not an `om-auto-*` skill: it chains no further skills and starts no implementation.
...
- **Never invent a reference.** A moodboard entry has a source (a link, a file, a product the user named) and a chooser. An agent-suggested reference is marked as a suggestion until the user keeps it.
