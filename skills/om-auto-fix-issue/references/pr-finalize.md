...
```
🤖 `om-auto-fix-issue` — 🏷️ label rationale: `review` (ready for code review) · `<category>` ({why this category}) · `needs-qa`|`skip-qa` ({why it needs / can skip manual QA}) · `priority-{level}` ({why this priority}) · `risk-{level}` ({why this risk})
```
