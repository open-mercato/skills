...
4. Consult the repository's agent instruction files (`AGENTS.md`, `CLAUDE.md`, or equivalents), `README`, the specs directory, `BACKWARD_COMPATIBILITY.md`, and — when `.uxproof/` exists (written by `om-ux-setup`) — `contract.json` and `conventions.md`. In `existing` mode these describe what exists or was specified and carry the `[PRODUCT]` tag; they do not establish user demand.
...
- **Write surface.** `product-brief.md`; decision records under `${research}/decisions/` written from confirmed choices with a name; direct personal accounts supplied in the session under `${research}/interviews/` with their role, date and limitations; and the capture templates the collection plan hands out under `${research}/templates/`. No commits: the invoking checkout holds the files; the routed skill (`om-prepare-issue`, `om-spec-writing`) or the user commits them.
