...
9. Coverage expansion (`sbst`), off by default. Ask only when step 2's repository inspection detects a coverage-expansion or mutation-testing tool already installed (a dependency/devDependency or lockfile entry — e.g. Stryker for JS/TS, Pynguin for Python, EvoSuite for Java, or an equivalent for the detected stack), or the user names one unprompted. If so, offer to record its run command(s) as `sbst.commands` and set `sbst.enabled: true`; otherwise leave `sbst` unset entirely — no question, no default row. Never install or suggest installing a new tool — this only wires up one the repo has already chosen. Skipped like every other question under `--defaults`. Full contract: `references/sbst.md`.
