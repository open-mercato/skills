...
6. **Check out the code to verify.**
   - **PR mode:** verify in an **isolated worktree**, never the primary one —
     ...
     `references/worktree-setup.md`. Before you `cd`, retain this loaded skill's
     absolute directory; resolve its later references from there and keep
     invoking sibling `om-*` skills by name.
