...
8. **Full validation gate before opening the PR.** Run every command in `validation.commands`, in order. Any non-zero exit fails the gate; fix and re-run until green. For **docs-only** runs (no code changes), the minimum gate is whatever configured command lints docs/markdown (if one exists) plus a manual re-read of the diff. Never skip the gate because an external skill suggested skipping it.

9. **Self-review.** Run the `om-code-review` skill against the branch diff and the breaking-change checklist: no silently broken public contract (checked against `BACKWARD_COMPATIBILITY.md` when present), no weakened security-sensitive surface, scope matching the plan. Fix findings and loop back to step 7. Full checklist: `references/review-report.md`.

10. **Open or reuse the PR and normalize labels.** Follow `references/pr-finalize.md`: prefer the `om-open-pr` skill when installed, fall back to the inline tracker operation **create-pr** against `$BASE_BRANCH` when it is not; never open a second PR when one exists for the branch; open **ready for review** (draft only for explicitly incomplete handoffs); use the PR body template (`references/pr-body-template.md`) with its mandatory `Tracking plan:` line; then apply the full label set (pipeline `review`, QA meta, category, exactly one priority, exactly one risk) through the `apply_label` guard with a short rationale comment per label.

11. **Run `om-auto-review-pr` and apply fixes.** Before the final summary comment, last pushes, or report, subject the PR to an automated second pass with `om-auto-review-pr` in autofix mode against `{prNumber}`, following its workflow verbatim: fixes as new commits in the same worktree (never rewriting history), re-run targeted validation (full step-8 gate when a fix reaches beyond a single module/test file), update the plan's Progress, and loop until a clean verdict or only documented non-actionable findings remain. It claims and releases its own `in-progress` lock — do not second-guess that. If it cannot run, leave `Status: in-progress`, stop, and report the blocker. Full procedure and verdict handling: `references/review-report.md`.

12. **Post the comprehensive summary comment.** End every run with a single summary comment on the PR that a human can read top-to-bottom without opening the diff, posted via the tracker operation **comment-pr** with a body file so formatting is preserved. Use the full structure — Summary of changes, External references honored, Verification phases completed, How to verify, What can go wrong — and its rules from `references/summary-comment-template.md`. Never post it before step 11 finishes, never claim a completion you did not reach, never paste secrets.

13. **Cleanup and lock release.** Always run cleanup in a finally/trap so crashes do not leak worktrees (the `git worktree remove --force` + `git worktree prune` sequence in `references/worktree-setup.md`, only when `CREATED_WORKTREE` is `1`). If the PR was opened, add a `PR: #{n}` line directly under the plan's `## Progress` heading (not a checklist line, so parsing is unaffected), commit, and push. Release any claim you hold per `references/claim-pr.md`.
...
- After the PR is open, run the `om-auto-review-pr` skill against it in autofix mode and keep applying fixes (as new commits, never as history rewrites) until it returns a clean verdict or only non-actionable findings remain — before pushing the final changes, posting the summary comment, and reporting back.
- Every run MUST end with the single comprehensive summary comment of step 12, with stable section headings across runs.
