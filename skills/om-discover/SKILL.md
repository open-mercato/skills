...
- Housekeeping (where the brief lands, who owns it, a missing name) is never one of the eight round questions; the round is for decisions. The hand-off in step 8 runs another skill only on an explicit yes, one step at a time, and never loops back into a second panel.
