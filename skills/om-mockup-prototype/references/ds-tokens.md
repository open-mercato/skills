...
1. `designTokens` in `<repo>/.ai/agentic.config.json`, when it names an existing repository-relative snapshot.
2. `<repo>/.ai/ds/ds-tokens.json` — the conventional committed snapshot.
3. `references/ds-tokens.default.json` — the default bundled with this skill.
...
Token names must be safe CSS custom-property identifiers. Values containing
declaration boundaries or `url()` are rejected so repository data cannot
escape the generated declaration or introduce a network fetch.
...
documented fields, or commits their own at the configured `designTokens` path
(default `.ai/ds/ds-tokens.json`), which always wins when present.
