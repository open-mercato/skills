...
2. Read `$TRACKER_FILE` — every tracker operation named in this skill executes as that descriptor defines. Read `SDLC.md` at the repo root for the Definition of Ready (its ticket-level tier gates step 1) and the label inference rules `om-prepare-issue` applies; when the repository has no `SDLC.md` yet, use the two-tier list this collection's own `SDLC.md` carries and say so.
