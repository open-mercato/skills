...
2. Read `$TRACKER_FILE` — every tracker operation and label guard named in this skill executes as that descriptor defines; a `BASE_BRANCH` of `"auto"` resolves via the **default-branch** operation. This skill uses: `RUNS_DIR`, `SPECS_DIR` (`paths.specs`, default `.ai/specs`), `LABELS_ENABLED`, `QA_GATE`, the validation commands; operations **current-user**, **get-pr**, **create-pr**, **comment-pr**, **assign-pr**, **label-pr**, **unlabel-pr**, **search-prs**, **list-prs**.
