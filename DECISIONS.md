...
Every worktree-owning skill `cd`s into an isolated checkout of the target project, which may not contain the installed skill or its `references/`. The portable fix is to retain the absolute directory of the `SKILL.md` the runner already selected, before changing directories, and resolve that skill's later files from it. Sibling skills remain invocations by name so the runner resolves them. Probing installation roots was rejected because runners discover different locations (and duplicate skill names can exist); the loaded file is the only authoritative anchor. Recording an install path in `.ai/agentic.config.json` was rejected because that file is per-project, while installation is per-machine. Copying or symlinking skills into the worktree was rejected because it dirties the target checkout and can be staged accidentally. The concise rule is synced across all nine worktree-owning skills.
