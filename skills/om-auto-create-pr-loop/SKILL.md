...
6. **Create an isolated worktree and task branch.** Work in an isolated worktree (never the primary; never nested) on the `feat/`/`fix/` branch from `origin/$BASE_BRANCH`, install dependencies, register `trap`/finally cleanup. Full bash: `references/worktree-setup.md`. Before you `cd`, retain this loaded skill's absolute directory; resolve its later references from there and keep invoking sibling `om-*` skills by name.
