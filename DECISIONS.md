...
## 2026-07-25 — Skill files resolve from the install root, never the worktree

Every worktree-owning skill `cd`s into an isolated worktree that is a checkout of the *target* project — which does not contain the `om-*` skills or their `references/` files. A relative `references/<file>.md` read therefore breaks the moment the skill enters the worktree, and the later steps (`pr-finalize`, `review-report`, `claim-pr`, …) all read references after that `cd`. The fix is a convention, not new config: pin the skills-install root before the `cd` — authoritatively the directory the agent already read this skill's `references/agentic-setup.md` from during setup, while CWD was still the invoking checkout — and read every later reference by that absolute path. Sibling skills keep working untouched because they are invoked *by name* through the Skill tool, which the harness resolves independent of CWD. A deterministic shell probe (walk the harness's skill roots — project `.claude/skills`, then user `~/.claude/skills`, then `~/.claude/plugins/*/skills`; first match wins, mirroring the harness's own resolution precedence) is documented for the rare case a shell value is needed. Two rejected alternatives: (1) recording the path in `.ai/agentic.config.json` — that file is per-project and committed, but the install location is a per-machine harness fact, so it does not belong there; (2) symlinking or copying the skills into the worktree — the link risks being staged into the target PR, adds `git status` noise, and breaks on `git worktree remove`. The note lives in each skill's `references/worktree-setup.md` (full, with the probe) and its SKILL.md worktree step (one clause), synced across all nine worktree-owning skills per the standard-step-file rule.
