...
2. **Create an isolated worktree and spec branch.** Never run in the user's primary worktree. Branch `spec/${SLUG}` detached from `origin/$BASE_BRANCH`; record `CREATED_WORKTREE` so any worktree you created is cleaned up in a trap/finally. Full commands: `references/worktree-setup.md`. Before you `cd`, retain this loaded skill's absolute directory; resolve its later references from there and keep invoking sibling `om-*` skills by name. When the brief names a handoff file (a `— brief: <path>` suffix from `om-brainstorm`), read it in the invoking checkout **before** creating the worktree and copy it in — a branch from `origin` does not contain it.
