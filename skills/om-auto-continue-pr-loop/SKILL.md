...
4. **Create an isolated worktree from the PR head.** Never resume in the user's primary worktree: create (or reuse) an isolated worktree from the PR head (`HEAD_REF`/`IS_CROSS` from the step 1 **get-pr**; use **checkout-pr** on the cross-repository path), install dependencies, and register `trap`/finally cleanup (only remove one you created). Full bash: `references/worktree-setup.md`. Before you `cd`, retain this loaded skill's absolute directory; resolve its later references from there and keep invoking sibling `om-*` skills by name.
