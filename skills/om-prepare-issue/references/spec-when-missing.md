...
`om-auto-implement-issue {issueId}` (both continue the same spec PR — never a
second one), or `om-auto-continue-pr {specPrNumber}` directly. Report both the
issue and the spec PR in step 5.

## Guardrails

- The PR is design-only. Delegate to `om-auto-write-spec` (which never
  implements) — never to `om-auto-implement-issue` / `om-auto-implement-spec` on
