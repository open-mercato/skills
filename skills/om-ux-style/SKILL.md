...
1. **Decide whether this skill applies.** Read `.uxproof/contract.json` when present. Declared tokens and a registered component set mean a design system exists → stop, say so, and route to `om-ux-setup` (`--refresh` when the user wants it re-extracted). A contract whose tokens are the *proposed palette* `om-ux-setup` derives from colors already in the code, or no contract at all, means this skill applies: the proposed palette becomes one input, never the answer.

2. **Gather the basis.** `product-brief.md` (Vision, Target group, Product and how it stands out, the benchmark table) when `om-discover` wrote one; the references the user brings (links, screenshots, products they admire and products they refuse to look like); the design directory's existing files. Each reference is recorded with who chose it and why, per `references/moodboard.md`. A basis with no references is a question to the user, not a moodboard the agent invents.
...
5. **Define the tokens** (`references/tokens.md`): color roles for both themes (ground, surface, text, muted, border, focus ring, primary with its hover and foreground, two accents, the semantic status roles kept as a contract), two type stacks and a scale, a spacing scale, radii, shadows. Named by role, never by value. Contrast is checked against WCAG AA before a pair is accepted.
...
7. **Write the contract** (`references/contract-writes.md`): `.uxproof/tokens.json` (declared, kind, source `design`), the manual section of `.uxproof/conventions.md` (principles, anti-patterns, component states, recipes — inside the manual markers, appended never replaced), a minimal `.uxproof/contract.json` when none exists, `${design}/theme.css` with the eight identity tokens and the semantic contract stated in its header, and `${design}/moodboard.md`. Show the diff before writing; on `--refresh`, superseded values stay in a *superseded* note.

8. **Run the quality gate** (`references/quality-gate.md`), then report per `references/report-templates.md` and end with the Output contract lines. Name the next command: `om-ux-setup --refresh` once components exist in code, so the extracted contract and the declared one converge.
...
Consumers parse `^Theme: (\S+)$` and `^Next: (none|om-[a-z-]+.*)$`.
...
- Interactive only — every principle, every token family, and the component list are confirmed by the user before they are written; this skill has no autonomous mode and must never be driven by an `om-auto-*` skill.
